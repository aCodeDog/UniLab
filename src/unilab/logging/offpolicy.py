...
class OffPolicyLogger(BaseTrainingLogger):
    ...
    def _get_replay_blocking_time(self) -> float:
        if self._learner_replay_wait_time > 0.0:
            return self._learner_replay_wait_time
        if self._iteration_time is None:
            return self._learner_incremental_h2d_time
        return 0.0

    def _get_learner_pipeline_time(self) -> float:
        return self._get_replay_blocking_time() + self._train_time + self._weight_sync_time
    ...
    def _build_timing_table(self) -> Table:
        ...
        if self._effective_batch_size > 0:
            system_items.append(("Eff Batch", f"{self._effective_batch_size:,}"))
        if self._learner_samples_per_iter > 0:
            system_items.append(("Learner Samples", f"{self._learner_samples_per_iter:,}"))
