...
def test_offpolicy_logger_terminal_timing_labels_include_wall_clock_and_distributed_context():
    ...
    assert "Eff Batch" in output
    assert "Learner Samples" in output
...
def test_offpolicy_logger_logs_wait_and_iter_throughput(monkeypatch):
    ...
    assert payload["perf/learner_pipeline_ms"] == pytest.approx(830.0)
    assert payload["perf/iter_ms"] == pytest.approx(10_900.0)
    assert payload["perf/iter_unaccounted_ms"] == pytest.approx(70.0)
...
def test_offpolicy_logger_tensorboard_logs_wall_clock_and_distributed_timing():
    ...
    assert scalars["perf/learner_pipeline_ms"] == pytest.approx(1_000.0)
